...
load_dotenv()
cash_flow_endpoint = os.getenv("CASH_FLOW_URL")
endpoint = os.getenv("WEBSITE_URL")

if cash_flow_endpoint == "https://payment-stage.ecpay.com.tw/Cashier/AioCheckOut/V5":
    merchant_id = "3002599"
...
# 建立金流訂單
def create_cash_flow_order(payment_methods: str, order_id: str, order_date: str, order_amount: int, product: str):
    ...
    TradeDesc = product #交易描述
    ItemName = product  #商品名稱，如果商品名稱有多筆，需在金流選擇頁一行一行顯示商品名稱的話，商品名稱請以符號#分隔
...
def create_checkMacValue(MerchantID, MerchantTradeNo, MerchantTradeDate, TotalAmount, TradeDesc, ItemName, ReturnURL, ChoosePayment, ClientBackURL):
    ...
    to_encode = f"HashKey={hash_key}&MerchantID={MerchantID}&MerchantTradeNo={MerchantTradeNo} \ 
    &MerchantTradeDate={MerchantTradeDate}&PaymentType=aio&TotalAmount={TotalAmount}&TradeDesc={TradeDesc} \ 
    ItemName={ItemName}&ReturnURL={ReturnURL}&ChoosePayment={ChoosePayment}&ClientBackURL={ClientBackURL}&EncryptType=1&HashIV={hash_iv}"
